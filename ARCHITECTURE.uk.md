...
8. Публікація через bearer-токен створює локальний реліз лише за відсутності
   module/version. Повтор тих самих bytes ідемпотентний, а інші bytes повертають
   `409 Conflict`. Явна заміна через manage UI вимагає глобального права
   видалення або такого права команди-власника; SQL атомарно перемикає реліз на
   новий content-addressed path і ставить попередній локальний object у надійну
   чергу видалення.
...
5. Cold body для `/v3/files/*` передається потоково безпосередньо з upstream у
   ...
   cache object.
6. Свіжі metadata upstream-модулів індексуються локально для вебінтерфейсу та метрик.
   JSON cache hit не повторює індексацію module/releases і не записує стан використання.
   Використання фіксують точні v3-запити релізів і файлів, а всі локальні та upstream-шляхи
   об'єднують SQL-записи до одного на реліз за хвилину на кожній репліці через обмежений
   in-memory throttle на 10 000 ключів. Спільна SQL лишається джерелом active-release стану.
