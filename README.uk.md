...
- `platform-admin` є зарезервованою configuration identity для global admin tokens та OIDC mappings; її не можна створити або використати як publishing team чи module space;
...
Після успішного завершення всіх publish jobs workflow оновлює source Helm chart metadata у default branch до випущених `version` і `appVersion`. Цей follow-up commit має `[skip ci]`, щоб звичайний branch CI не запускався тільки через release metadata.
