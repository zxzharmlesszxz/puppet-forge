...
5. If Forge advertises a release before its `/v3/releases/...` metadata becomes available, the service verifies `/v3/files/...tar.gz`, derives both checksums from that artifact, and serves a complete local release response instead of exposing the transient metadata 404 to r10k.
...
| `RELEASE_CONSUMER_TTL`            | `4320h`                             | no                               | Retention for named access-token observations of concrete module archive downloads. Set to `0s` to retain observations without automatic cleanup.                                                                                                                                                      |
...
| `UPSTREAM_URL`                    | `https://forgeapi.puppet.com`       | no                               | Upstream Puppet Forge API used when a module is not available locally.                                                                                                                                                                                                                                 |
...
| `METRICS_RELEASE_CONSUMER_LIMIT`  | `10000`                             | no                               | Maximum number of token/module/version consumer records exported in the cached metrics snapshot.                                                                                                                                                                                                       |
...
The workflow first verifies that committed Helm `version` and `appVersion` match the release tag. It then creates a draft GitHub Release, publishes and signs the immutable Docker version, and publishes the unmodified chart from the tagged commit. After every versioned component succeeds, it promotes `:latest` and makes the GitHub Release public.
