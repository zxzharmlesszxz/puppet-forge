...
func TestPrincipalCanDeleteOwnerUsesManagedTeamsNotPublishOwners(t *testing.T) {
	...
	if principal.CanDeleteOwner("alpha") {
...
func TestAuthenticateOIDCMergesMultipleTeamAdminMappings(t *testing.T) {
	...
	authorizer, err := NewAuthorizer([]TeamConfig{
		...
		},
		{
			...
			PublishOwners:       []string{"alpha"},
			OIDCTeamAdminEmails: []string{"owner@example.com"},
			OIDCTeamAdminGroups: []string{"alpha-admins"},
		},
	})
	if err != nil {
		t.Fatalf("NewAuthorizer() error = %v", err)
	...
	principal, ok := authorizer.AuthenticateOIDC("OWNER@example.com", "", []string{"teamname-admins", "alpha-admins"})
	if !ok {
		t.Fatal("expected OIDC principal")
	}
	...
	if _, ok := principal.ManagedTeams["alpha"]; !ok {
		t.Fatalf("principal cannot manage alpha: %#v", principal.ManagedTeams)
	}
	for _, owner := range []string{"teamname", "shared", "alpha"} {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			configs: []TeamConfig{
				...
				{Team: "alpha", PublishTokens: []string{"shared-secret"}},
			},
		},
		{
