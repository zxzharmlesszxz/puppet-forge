...
func (s *PostgresStore) ensureOperationalTables(ctx context.Context) error {
	...
	migrateOIDC, err := s.needsAccessOIDCMappingsMigration(ctx)
	if err != nil {
		return err
	}
	if migrateOIDC {
		if err := s.migrateAccessOIDCMappings(ctx); err != nil {
			return err
		}
	}
	return nil
...
func (s *PostgresStore) needsAccessOIDCMappingsMigration(ctx context.Context) (bool, error) {
...
func (s *PostgresStore) migrateAccessOIDCMappings(ctx context.Context) error {
	const query = `
