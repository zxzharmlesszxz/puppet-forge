...
Global administrators can inspect current legacy signals at `/manage/admin/legacy`. Team administrators have the same view scoped to tokens owned by their team at `/manage/teams/<team>/legacy`. Each view selects the most recently observed version for every named token and module directly from retained SQL history, uses server-side filtering and pagination, and is not constrained by `METRICS_RELEASE_CONSUMER_LIMIT`, which only bounds the Prometheus snapshot.
