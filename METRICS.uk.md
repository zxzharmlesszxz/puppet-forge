...
Глобальні адміністратори можуть переглядати поточні legacy-сигнали на сторінці `/manage/admin/legacy`. Адміністратори команд мають аналогічну сторінку `/manage/teams/<team>/legacy`, обмежену токенами їхньої команди. Кожна сторінка безпосередньо зі збереженої SQL-історії вибирає останню спостережену версію для кожного іменованого токена й модуля, використовує серверну фільтрацію та пагінацію і не обмежується значенням `METRICS_RELEASE_CONSUMER_LIMIT`, яке визначає лише максимальний розмір Prometheus snapshot.
